...
func make_rooms():
	...
	for room in $Rooms.get_children():
		if randf() < cull:
			...
			room.disable_collisionshape2d()
			room_positions.append(Vector3(room.position.x, room.position.y, 0))
	yield(get_tree(), "idle_frame")
	# generate a minimum spanning tree connectiong the rooms
...
func find_mst(nodes):
	...
	# repeat untill no more nodes remain in array nodes
	while nodes:
		...
		for p1 in path.get_points():
			...
			# Loop throught the remaining nodes
			for p2 in nodes:
...
func carve_path(pos1, pos2):
	# Carve a path between to points
	...
	for x in range(pos1.x, pos2.x, x_diff):
		...
		Map.set_cell(x, x_y.y + y_diff, 0) # Widen the corridors
	for y in range(pos1.y, pos2.y, y_diff):
		Map.set_cell(y_x.x, y, 0)
		Map.set_cell(y_x.x + x_diff, y, 0) # Widen the corridors
